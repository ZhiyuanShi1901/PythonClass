...
class CoraData(object):
    ...
    def __init__(self, data_root="cora", rebuild=False):
        f"""包括数据下载、处理、加载等功能
        当数据的缓存文件存在时，将使用缓存文件，否则将下载、处理、并保存到磁盘
    ...
    def process_data(self):
        """
        处理数据，得到节点特征和标签， 邻接矩阵，徐年纪、验证集以及测试集
        """
        ...
        y = np.concatenate((allx, ty), axis=0).argmax(axis=1)
...
indices = torch.from_numpy(
    np.asarry([normalize_adjacency.row,
...
def train():
    ...
    for epoch in range(epochs):
        ...
        val_acc_history.append(loss.item())
