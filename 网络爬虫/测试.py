...
def baiduyunbaocun(yiyi, page=1, lastnum=100):
    ...
    if lastnum == 100:  # 如果初始值是100，则使用yiyi长度
        ...
        lastnumed = lastnum
    print(f'yiyi  {len(yiyi)}')
    print(f'lastnumed {lastnumed}')
    number = 0+first
    for web in yiyi[first:lastnumed]:
        print(number)
        web.click()
...
# sleep(10)
# 顺序保存
jingpin = bro.find_element_by_xpath('//*[@id="category_0"]/table/tbody/tr[1]/td[2]/h2/a')
jingpin.click()
sleep(1)
...
lastnum = input('请输入最后一页不不不不需要保存的个数。如果最后一页需要全部保存，请输入0。')
pages, lastnum = eval(pages), eval(lastnum)
num = 0
page = 1
print(f'while {lastnum}')
while(page <= pages):

    yiyi = bro.find_elements_by_class_name('s.xst')

    if lastnum != 0 and page == pages:
        print(f'yiyinei {lastnum}')
        baiduyunbaocun(yiyi, page, lastnum)

    else:
        print('baiduyunbaocun2')
        baiduyunbaocun(yiyi, page=page)
