...
def baiduyunbaocun(yiyi, page=1):
    if page == 1:
        first = 7  # 如果是第一页，忽略顶部的几个链接。
    else:
        first = 0
    for web in yiyi[first:len(yiyi)]:
        web.click()
        print('点击完成')
...
# sleep(10)
# 顺序保存
jingpin = bro.find_element_by_xpath('/html/body/div[8]/div[3]/div[2]/div[3]/div[1]/div[2]/table/tbody/tr[1]/td[2]/h2/a')
jingpin.click()
sleep(2)
...
while(last_one <= first_one):
    webs = []
    for i in range(0, 30):
        yiyi = bro.find_elements_by_xpath(f'//*[@id="normalthread_{last_one}"]/tr/th/a[2]')
        last_one = last_one - 1
        webs.extend(yiyi)
    print(webs)
    baiduyunbaocun(webs, page)
    sleep(2)
    # 点击下一页
